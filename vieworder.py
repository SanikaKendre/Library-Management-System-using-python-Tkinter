# -*- coding: utf-8 -*-
"""
Created on Sun Nov 19 17:37:22 2023
...
cursor = db.cursor()
create_table ='''CREATE TABLE IF NOT EXISTS vieworder(Book_Id INTEGER);'''
...
def insert():
    ...
    if (not isinstance(bid,int) or bid == 0):
       ms.showinfo("Message", "please enter valid Bookid")
        ...
        db.commit()
        ms.showinfo("view order succesfully")
    else:
...
view_button=Button(root,command=insert,text="VIEW",bg="#FF7D40",width=10,height=1,fg="#CD2626",font=("times",15,"bold"))
